...
def main(
    proc_dir: Path = typer.Option(
        PROCESSED_DATA_DIR, "--proc-dir", "-p", help="Directory for data"
    ),
    ...
    """
    data_dir: Path = proc_dir / Path(folder)
    output_dir: Path = data_dir / Path("pdf")
    output_dir.mkdir(parents=True, exist_ok=True)

    for zone_type in tree[folder]:
        for zone in tree[folder][zone_type]:
            # Files
            input_file: Path = (data_dir / Path(zone)).with_suffix(".json")
            if not input_file.exists():
                logger.info(f"Fichier inexistant: {input_file}")
                continue
